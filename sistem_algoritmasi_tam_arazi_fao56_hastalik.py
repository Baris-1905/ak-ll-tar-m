...
if secili_mahalle and secili_urun and arazi:
    ...
    st.subheader("🦠 Hastalık ve İlaç Önerisi (Gelişmiş Analiz)")
    bulunan = False
    for veri in hastalik_veritabani[secili_urun]:
        min_t, max_t = veri["risk_araligi"]
        if min_t <= sicaklik <= max_t and rakim >= veri["rakim_min"]:
            doz = veri["doz"] * arazi
            st.error(f"Risk: {veri['ad']} - İlaç: {veri['ilac']} - Toplam Doz: {doz:.2f} L")
            bulunan = True
    if not bulunan:
        st.success("Belirgin hastalık riski tespit edilmedi.")
